...
//void Carta::voltear() { if (bocaAbajo != true) { setBocaAbajo(false); } else { setBocaAbajo(true) } }
int Carta::getNum() { if (num>0 && num < 11) { return num; } else return 10; }

std::string Carta::toString() {//Método toString
	if (bocaAbajo == true) return "Carta Boca Abajo";
	std::stringstream s;
	if (getBocaAbajo() == true) {
		s << "Carta boca abajo" << '\n';
	}
		switch (num)
		{
